...
void Mira::setRot(float r)
{
	rot = r;
}

void Mira::Inicializar(string name, string path, int nAnims, int nFrames)
{
	...
	aimSprite.setSpriteSheet(name); //Definimos o SpriteSheet do Sprite
...
void Mira::Desenhar()
{
	if (ativo)
	{
		aimSprite.desenhar(x, y, rot); //Desenhamos a flecha na respectiva posição e rotação
	}
}
...
void Mira::Atirar(int _x, int _y, float _force)
{
